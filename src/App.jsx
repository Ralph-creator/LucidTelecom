...
function Register({ onBackToLogin, darkMode, onToggleTheme }) {
  const [form, setForm] = useState({ fullName: '', username: '', email: '', phone: '', pin: '', referral: '', password: '' })
  ...
  const submit = (event) => {
    ...
    setNotification(`Account created successfully for ${form.username}. You can now sign in.`)
    setTimeout(onBackToLogin, 2600)
  }

  return <div className="login-page min-h-screen bg-slate-50 p-3 text-slate-900 sm:p-5 lg:p-7">
    {notification && <div role="status" className="success-toast fixed right-4 top-4 z-50 flex max-w-sm items-start gap-3 rounded-2xl border border-emerald-200 bg-white px-4 py-3 text-sm font-semibold text-emerald-700 shadow-xl shadow-emerald-100"><Check size={18} className="mt-0.5 shrink-0" /><span>{notification}</span></div>}
    <div className="mx-auto grid min-h-[calc(100vh-24px)] max-w-[1440px] overflow-hidden rounded-[28px] bg-white shadow-2xl shadow-slate-300/30 lg:grid-cols-[.8fr_1.2fr] lg:rounded-[36px]">
      <section className="relative hidden overflow-hidden bg-slate-950 p-9 text-white lg:flex lg:flex-col xl:p-12">
        ...
        <div className="relative my-auto max-w-sm"><p className="text-sm font-semibold uppercase tracking-[.16em] text-emerald-300">Join LucidTelecom</p><h1 className="mt-4 text-4xl font-extrabold leading-tight xl:text-5xl">Get started <span className="text-emerald-400">absolutely free.</span></h1><p className="mt-5 text-base leading-7 text-slate-300">Create your account and manage data, airtime, bills and payments from one secure dashboard.</p></div>
      ...
      <section className="flex min-h-[calc(100vh-24px)] flex-col p-6 sm:p-10 lg:min-h-0 lg:p-12 xl:p-16">
        <div className="flex items-center justify-between"><div className="flex items-center gap-2 lg:hidden"><div className="grid h-9 w-9 place-items-center rounded-xl bg-emerald-500 text-lg font-black text-white">L</div><span className="text-lg font-extrabold tracking-tight">Lucid<span className="text-emerald-500">Telecom</span></span></div><div className="flex items-center gap-4"><ThemeToggle darkMode={darkMode} onToggle={onToggleTheme} /><p className="hidden text-sm text-slate-500 sm:block">Already have an account? <button onClick={onBackToLogin} className="font-bold text-emerald-600 hover:text-emerald-700">Login</button></p></div></div>
        <div className="mx-auto my-auto w-full max-w-2xl py-8"><p className="text-sm font-semibold text-emerald-600">WELCOME TO LUCIDTELECOM</p><h2 className="mt-2 text-3xl font-extrabold tracking-tight">Create your account</h2><p className="mt-3 text-sm leading-6 text-slate-500">Set up your account in a few moments and start using every essential service.</p>
          <form onSubmit={submit} className="mt-7 grid gap-4 sm:grid-cols-2"><label className="block text-sm font-bold text-slate-700">Full name<input required value={form.fullName} onChange={updateField('fullName')} placeholder="Your full name" className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium outline-none transition placeholder:text-slate-400 focus:border-emerald-400 focus:bg-white focus:ring-4 focus:ring-emerald-50" /></label><label className="block text-sm font-bold text-slate-700">Username<input required value={form.username} onChange={updateField('username')} placeholder="Choose a username" className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium outline-none transition placeholder:text-slate-400 focus:border-emerald-400 focus:bg-white focus:ring-4 focus:ring-emerald-50" /></label><label className="block text-sm font-bold text-slate-700">Email address<input required type="email" value={form.email} onChange={updateField('email')} placeholder="you@example.com" className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium outline-none transition placeholder:text-slate-400 focus:border-emerald-400 focus:bg-white focus:ring-4 focus:ring-emerald-50" /></label><label className="block text-sm font-bold text-slate-700">Phone number<input required type="tel" value={form.phone} onChange={updateField('phone')} placeholder="080 1234 5678" className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium outline-none transition placeholder:text-slate-400 focus:border-emerald-400 focus:bg-white focus:ring-4 focus:ring-emerald-50" /></label><label className="block text-sm font-bold text-slate-700">Transaction PIN<input required inputMode="numeric" pattern="[0-9]{4}" maxLength="4" value={form.pin} onChange={updateField('pin')} placeholder="4-digit PIN" className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium outline-none transition placeholder:text-slate-400 focus:border-emerald-400 focus:bg-white focus:ring-4 focus:ring-emerald-50" /></label><label className="block text-sm font-bold text-slate-700">Referral username <span className="font-normal text-slate-400">(optional)</span><input value={form.referral} onChange={updateField('referral')} placeholder="Referral username" className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium outline-none transition placeholder:text-slate-400 focus:border-emerald-400 focus:bg-white focus:ring-4 focus:ring-emerald-50" /></label><label className="relative block text-sm font-bold text-slate-700 sm:col-span-2">Password<div className="relative mt-2"><input required minLength="6" type={showPassword ? 'text' : 'password'} value={form.password} onChange={updateField('password')} placeholder="Create a password" className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 pr-12 text-sm font-medium outline-none transition placeholder:text-slate-400 focus:border-emerald-400 focus:bg-white focus:ring-4 focus:ring-emerald-50" /><button type="button" onClick={() => setShowPassword(!showPassword)} aria-label={showPassword ? 'Hide password' : 'Show password'} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-lg p-1.5 text-slate-400 hover:bg-slate-100">{showPassword ? <EyeOff size={18} /> : <EyeClosed size={18} />}</button></div></label><button className="sm:col-span-2 w-full rounded-xl bg-emerald-600 py-3.5 text-sm font-bold text-white shadow-lg shadow-emerald-200 transition hover:-translate-y-0.5 hover:bg-emerald-700 active:translate-y-0">Create account</button><p className="sm:col-span-2 text-xs leading-5 text-slate-500">By creating an account, you agree to LucidTelecom’s <button type="button" className="font-semibold text-slate-700 underline">Terms of Service</button> and <button type="button" className="font-semibold text-slate-700 underline">Privacy Policy</button>.</p></form>
...
function Login({ onLogin, onRegister, darkMode, onToggleTheme }) {
  ...
  return <div className="login-page min-h-screen bg-slate-50 p-3 text-slate-900 sm:p-5 lg:p-7">
    <div className="mx-auto grid min-h-[calc(100vh-24px)] max-w-[1440px] overflow-hidden rounded-[28px] bg-white shadow-2xl shadow-slate-300/30 lg:grid-cols-[1.05fr_.95fr] lg:rounded-[36px]">
      ...
      <section className="flex min-h-[calc(100vh-24px)] flex-col p-6 sm:p-10 lg:min-h-0 lg:p-12 xl:p-16">
        <div className="flex items-center justify-between lg:justify-end"><div className="flex items-center gap-2 lg:hidden"><div className="grid h-9 w-9 place-items-center rounded-xl bg-emerald-500 text-lg font-black text-white">L</div><span className="text-lg font-extrabold tracking-tight">Lucid<span className="text-emerald-500">Telecom</span></span></div><div className="flex items-center gap-4"><ThemeToggle darkMode={darkMode} onToggle={onToggleTheme} /><p className="hidden text-sm text-slate-500 sm:block">New to LucidTelecom? <button onClick={onRegister} className="font-bold text-emerald-600 hover:text-emerald-700">Create account</button></p></div></div>
...
function App() {
  ...
  return <div className={darkMode ? 'theme-dark' : ''}>{authenticated ? <Dashboard onLogout={() => setAuthenticated(false)} darkMode={darkMode} onToggleTheme={toggleTheme} /> : registering ? <Register onBackToLogin={() => setRegistering(false)} darkMode={darkMode} onToggleTheme={toggleTheme} /> : <Login onLogin={() => setAuthenticated(true)} onRegister={() => setRegistering(true)} darkMode={darkMode} onToggleTheme={toggleTheme} />}</div>
}
